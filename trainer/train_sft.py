...
2. 加载预训练权重: model.load_state_dict()
3. collate_fn: 自定义批处理函数，用于padding
4. pad_sequence: 将变长序列填充到相同长度
5. 忽略prompt损失: 使用-100标签
"""
...
def train_sft_epoch(model, train_loader, val_loader, optimizer, device, num_epochs, start_context, tokenizer, eval_freq=1000, eval_iter=5):
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, target_batch in train_loader:
            ...
            loss = calc_sft_loss_batch(input_batch, target_batch, model, device)  # 计算SFT损失
            loss.backward()  # 反向传播
            optimizer.step()  # 更新参数
            steps += 1
...
def collate_fn(batch):
    ...
    相关知识：
    - SFT数据中不同样本长度不同，需要padding
    - input_ids用0填充
    - labels用-100填充（损失计算时会被忽略）
    """
    input_ids, labels = zip(*batch)

    # padding_value=0: 用0填充input_ids
    input_ids_padded = pad_sequence(input_ids, batch_first=True, padding_value=0)

    # padding_value=-100: 用-100填充labels（损失计算时忽略）
